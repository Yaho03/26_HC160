...
class EnrollmentSplitTest(unittest.TestCase):
    def test_first_session_is_enrollment(self):
        rows = _rows("p01", "s2", "clean", [0.1]) + _rows("p01", "s1", "clean", [0.2])
        enrollment, test = enrollment_split(rows, "p01")

        self.assertEqual({r["session_id"] for r in enrollment}, {"s1"})
        self.assertEqual({r["session_id"] for r in test}, {"s2"})
    ...
    def test_single_session_subject_cannot_be_split(self):
        ...
        with self.assertRaises(NoEnrollmentSessionError):
            enrollment_split(rows, "p01")

    def test_split_is_deterministic(self):
        rows = _rows("p01", "sb", "clean", [0.1]) + _rows("p01", "sa", "clean", [0.2])
        first = enrollment_split(rows, "p01")[0][0]["session_id"]
        second = enrollment_split(list(reversed(rows)), "p01")[0][0]["session_id"]
