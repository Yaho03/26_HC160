...
def main() -> None:
    ...
    if attack_index.empty:
        raise ValueError("No attack rows selected for bit-depth reduction defense")
    ...
    for _, attack in tqdm(attack_index.iterrows(), total=len(attack_index), desc=f"bitdepth {args.bits}bit"):
        ...
        if not adv_path.exists():
            rows.append({
                ...
                "defense": "bit_depth",
                "defense_params": json.dumps({"bits": args.bits}),
        ...
        defended = reduce_bit_depth(image, args.bits)
        defended_path = image_out_dir / f"{attack['sample_id']}_bitdepth_{args.bits}bit.png"
