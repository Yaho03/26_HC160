...
`.github/workflows/face-auth.yml`은 Python 3.11에 `requirements-face-auth.txt`를 설치한 뒤
unit·integration test를 실행한다. ML 의존성이 없는 research-contract workflow와 분리되어
있으며 model checkpoint를 다운로드하거나 실제 camera를 사용하지 않는다.
