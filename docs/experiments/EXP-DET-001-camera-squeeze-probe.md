...
### 17.2 원인은 등록 세션의 조명 편향

피험자별 등록 세션과 테스트 세션의 clean 통계다. `jpeg_q75` self_consistency 기준이다.

| 피험자 | 등록 평균 | 테스트 평균 | 테스트/등록 |
|---|---|---|---|
| p01 | 0.00355 | 0.00258 | 0.73배 |
| p02 | 0.00517 | 0.00340 | 0.66배 |
| p03 | 0.00214 | 0.00253 | **1.19배** |

`p01`과 `p02`는 테스트 세션 점수가 등록보다 낮아, 자기 등록 통계로 정규화하면 음수
쪽으로 치우쳐 임계값을 넘지 않는다. `p03`은 반대로 높아 양수 쪽으로 밀린다. 임계값은
`p01`과 `p02`에서 학습되므로 그 둘의 편향에 맞춰지고, `p03`에 적용하면 어긋난다.

근본 원인은 등록 세션이 조명 조건 하나만 담는 것이다. 촬영 프로토콜은 1회차 밝은 곳,
2회차 어두운 곳으로 지정했고 등록에는 첫 세션만 쓴다. 따라서 등록 통계가 한 조명만
대표한다.

실제 배포에서도 등록은 한 번이지만, 그렇다면 등록 시점에 조건 변화를 포함시켜야 한다.
등록 절차에서 조명이나 자세를 바꿔가며 여러 구간을 수집하는 것이 대안이다.

### 17.3 판단

`per_user`가 `population`보다 평균 TPR이 2.2배 높다. 방향은 여전히 맞다.

그러나 어느 쪽도 배포 가능한 상태가 아니다. `population`은 한 피험자에서 탐지율 0%,
`per_user`는 한 피험자에서 오탐률 49.4%다. `07` 7절의 두 기준을 3-fold 전체에서
만족하는 구성이 없다.

표본이 늘 때마다 결과가 나빠지는 경향이 이어진다. 1세션 67.2%, 2세션 28.1%,
4세션 2인 38.7%, 6세션 3인에서는 fold마다 갈린다. 이는 이전 결과들이 좁은 조건에서
얻어졌다는 뜻이며, 넓힐수록 실제에 가까워진다.
