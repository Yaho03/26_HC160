...
협업 작업을 추적할 때는 다음 작업 ID를 사용할 수 있다. 이 ID는 연구 요구사항 ID를
대체하지 않으며, 연결된 GitHub 이슈에서 범위와 완료 조건을 정의해야 한다.

| 접두어 | 영역 |
|---|---|
| `FOR-XXX` | 공격 결과 포렌식·위험 점수 |
| `REP-XXX` | 보고서·대시보드 전달 산출물 |
| `CI-XXX` | CI·저장소 통합 작업 |
| `DOC-XXX` | 문서 운영 규칙 |
...
브랜치는 이슈에서 만들고, 다음 형식을 사용한다.

```text
component/REQUIREMENT-OR-WORK-ID-short-name
```

컴포넌트는 코드와 문서의 실제 소유 영역을 나타낸다.

```text
verification/VER-003-bind-threshold-version
...
infra/CI-002-main-history-recovery
experiment/EXP-ATK-002-pgd-seed-evaluation
docs/DOC-003-component-branch-rules
```

허용하는 최상위 컴포넌트는 `verification`, `attack`, `defense`, `face-auth`,
`forensics`, `reports`, `infra`, `experiment`, `docs`다. 변경 타입이나 개인·자동화
도구 이름을 접두사로 사용하지 않는다. 버그 수정은 별도 `fix` 접두사를 만들지 않고
수정 대상 컴포넌트에 둔다.
