...
| Adversarial inspection | 독립 optional veto 2종으로 구현. 계측·분석·threshold artifact·방어 전후 비교·adaptive attack(EOT, BPDA)·피험자 분리 검증 구현됨. 등록 세션은 촬영 시각으로 고르며 두 배정의 민감도를 함께 보고한다(EXP-DET-001 17.1.1) | 두 게이트와 정책 통합 test, `squeeze_probe`·`probe_log`·`probe_capture`·`probe_analyze`·`probe_threshold`·`probe_merge`·`conditional_asr`·`adaptive_attack`·`bpda`·`cross_subject`와 단위 test | 사용자당 세션 3개 이상 수집(등록 배정 민감도 해소), 임계값 전이 방식 확정, BPDA 대응, 하드코딩 상수 교체, latency 평가 |
