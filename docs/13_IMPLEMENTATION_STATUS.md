...
| 버전 | v1.1 |
| 상태 | 진행 중 |
| 기준 snapshot | commit `a5fe5ad`, branch `face-auth/ARCH-001-realtime-face-auth` |
| 최종 수정일 | 2026-08-23 |
...
| Adversarial inspection | 독립 optional veto 2종으로 구현. 계측·분석·threshold artifact·방어 전후 비교·adaptive attack(EOT, BPDA)·피험자 분리 검증 구현됨 | 두 게이트와 정책 통합 test, `squeeze_probe`·`probe_log`·`probe_capture`·`probe_analyze`·`probe_threshold`·`probe_merge`·`conditional_asr`·`adaptive_attack`·`bpda`·`cross_subject`와 단위 test | 피험자 3인 이상 수집, 사용자별 임계값 또는 정규화, BPDA 대응, 하드코딩 상수 교체 |
| Detector latency | 1차 측정 완료·예산 판정 불가 | `latency_bench` CLI와 단위 test. M1 Pro(Rosetta 2) 단일 기기에서 두 경로를 장치별 2 run x 50 표본으로 측정. face_auth 게이트 mps p50 616 ms / p95 643 ms, 연구 프로브 mps p50 85 ms / p95 95 ms. 병목은 변환(게이트 total의 1%)이 아니라 배치 1짜리 forward 9회. `docs/experiments/PERF-001-detector-latency.md` | `07` 7절 latency budget 수치가 규범 문서에 없어 충족 판정 불가. reference hardware 지정, 캡처 FPS와 frame drop 측정, 게이트 forward 배치화 |
