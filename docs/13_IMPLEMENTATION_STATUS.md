...
| Face-auth CI | 구현됨 | Python 3.11, 고정 dependency 설치, `pip check`, unit·integration suite workflow | Model artifact, 실제 camera, ONNX와 target-device 성능은 별도 검증 필요 |
...
| P1 | Python 3.11 CI 검증은 존재하지만 `requirements-face-auth.txt`는 version pin만 있고 hash lock이 없음 | Hash-locked environment artifact를 공개하고 optional runtime·model 검증을 별도 수행 |
