...
| Adversarial inspection | Optional veto로 구현. 계측·분석·artifact 생성 구현됨 | Transform-consistency와 feature-squeeze module, `squeeze_probe`·`probe_log`·`probe_capture`와 단위 test | 웹캠 세션 실행, clean-only threshold 산출, threshold artifact 생성, adaptive attack, latency 평가 |
