...
| Adversarial inspection | 독립 optional veto 2종으로 구현. 계측·분석·threshold artifact·방어 전후 비교·adaptive attack(EOT, BPDA)·피험자 분리 검증 구현됨 | 두 게이트와 정책 통합 test, `squeeze_probe`·`probe_log`·`probe_capture`·`probe_analyze`·`probe_threshold`·`probe_merge`·`conditional_asr`·`adaptive_attack`·`bpda`·`cross_subject`와 단위 test | 피험자 3인 이상 수집, 사용자별 임계값 또는 정규화, BPDA 대응, 하드코딩 상수 교체, latency 평가 |
